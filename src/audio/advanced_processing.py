...
class AdvancedTranscriber:
    ...
    def _get_cache_key(self, audio_file: str, start_time: float = 0, end_time: float = 0) -> str:
        """캐시 키 생성"""
    ...
    def _is_cached(self, audio_file: str, start_time: float = 0, end_time: float = 0) -> Optional[Dict[str, Any]]:
        """캐시된 결과 확인"""
    ...
    def _save_to_cache(self, audio_file: str, result: Dict[str, Any], start_time: float = 0, end_time: float = 0):
        """결과를 캐시에 저장"""
        if not self.enable_cache:
            return
            ...
            while current_time < duration:
                ...
                if end_time < duration:
                    ...
                    if silence_ranges:
                        ...
                        if optimal_split > current_time + self.max_chunk_duration * 0.8:  # 80% 이상이면
    ...
    def _transcribe_chunk_with_retry(self, audio_file: str, start_time: float, end_time: float) -> Dict[str, Any]:
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                # 캐시에 저장
                self._save_to_cache(audio_file, transcript_result, start_time, end_time)
    ...
    def transcribe_advanced(self, audio_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(chunks) == 1:
                # 단일 chunk 처리
                result = self._transcribe_chunk_with_retry(audio_file, chunks[0][0], chunks[0][1])
            else:
                # 병렬 chunk 처리
                ...
                for start, end in chunks:
                    future = self.executor.submit(self._transcribe_chunk_with_retry, audio_file, start, end)
