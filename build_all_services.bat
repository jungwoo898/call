@echo off
echo 🚀 Callytics 마이크로서비스 전체 빌드 시작
echo ==========================================

echo.
echo 🧹 기존 이미지 정리 중...
docker images | findstr callytics > nul
if %errorlevel% equ 0 (
    echo 기존 callytics 이미지들을 제거합니다...
    docker rmi $(docker images | findstr callytics | awk "{print $3}") 2>nul
    echo 기존 이미지 정리 완료
) else (
    echo 기존 이미지가 없습니다.
)
